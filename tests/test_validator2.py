...
def run_test(test_name):
    """
    Main test flow:
    1. Load input keys from OSI file
    2. Pass to calculation 
    3. Get results and compare with Excel expected values
    """
    # Step 1: Load input keys from OSI file
    osi_file_path = os.path.join(OSI_DIR, f"{test_name}.osi")
    osi_data = load_osi_file(osi_file_path)
    
    # Step 2: Convert OSI data to design dictionary (Input keys)
